...
const Product: React.FC = () => {
  ...
  const getProducts = () => {
    ...
    axios
      .get(`http://localhost:8081/products/all-product-details`)
      .then((response) => {
        console.log(response.data);
        response.data.map((x: { [x: string]: any; id: any }) => {
          x["key"] = x.id;
  ...
  const modalFormSubmit = async () => {
    try {
      ...
      if (modalState === "CREATE") {
        ...
        axios
          .put(`http://localhost:8081/products/${productId}`, {
            code: productForm.getFieldValue("code"),
            name: productForm.getFieldValue("name"),
            summary: productForm.getFieldValue("summary"),
            description: productForm.getFieldValue("description"),
  ...
  const productColumns: ColumnsType<IProduct> = [
    ...
    },
    {
      ...
      key: "createdDate",
      render: (_, record) =>
        moment.utc(record.createdDate).local().format(dateFormat),
    },
    {
      ...
      render: (_, record) =>
        moment.utc(record.lastModifiedDate).local().format(dateFormat),
    },
    {
      title: "Action",
      key: "action",
